...
function renderStreakCard(stats, options = {}) {
  ...
  const animate = options.animate !== "false" && options.animate !== false;
  ...
  const year = recentDays.length > 0 && recentDays[recentDays.length - 1]
    ? new Date(recentDays[recentDays.length - 1].date).getFullYear()
  ...
  for (let col = 0; col < cols; col++) {
    const day = padded.slice(col * 7, col * 7 + 7).find(Boolean);
    if (!day) continue;
  ...
  // Day-of-week labels (row index 0 = Sunday, per GitHub calendar convention)
  const dayLabels = [
    { row: 1, label: "Mon" },
    { row: 3, label: "Wed" },
    { row: 5, label: "Fri" },
  ...
  const width = Math.max(495, gridX + cols * stepX + halfW + 40);

  return `<svg width="${width.toFixed(0)}" height="${height.toFixed(0)}" viewBox="0 0 ${width.toFixed(0)} ${height.toFixed(0)}" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="${esc(stats.displayName)}'s contribution graph">
...
<rect width="${width.toFixed(0)}" height="${height.toFixed(0)}" rx="14" fill="url(#bg)"/>
...
<text x="${(width - 290).toFixed(0)}" y="${bottomY.toFixed(0)}" font-size="11" font-family="sans-serif">
  <tspan fill="${ACCENT}" font-weight="600">${stats.currentStreak}</tspan>
  <tspan fill="${theme.sub}"> day streak</tspan>
</text>

<g transform="translate(${(width - 24).toFixed(0)},${(bottomY - 6).toFixed(0)})">
